...
void SceneGraph::onRead(oishii::BinaryReader& reader, BMDOutputContext& ctx)
{
	...
	for (ByteCodeCmd cmd(reader); cmd.op != ByteCodeOp::Terminate; cmd.transfer(reader))
	{
		...
		case ByteCodeOp::Joint: {
			...
			{
				ctx.mdl.getBones()[ctx.jointIdLut[joint_stack.back()]].children.emplace_back(ctx.mdl.getBones()[ctx.jointIdLut[newId]].id);
				ctx.mdl.getBones()[ctx.jointIdLut[newId]].parent = ctx.mdl.getBones()[ctx.jointIdLut[joint_stack.back()]].id;
			}
			joint = newId;
			break;
		}
		...
		case ByteCodeOp::Shape:
			if (mat < ctx.mdl.getMaterials().size())
				ctx.mdl.getBones()[ctx.jointIdLut[joint]].displays.emplace_back(ctx.mdl.getMaterials()[mat].id, cmd.idx);
			break;
		case ByteCodeOp::Unitialized:
		default:
			assert(!"Invlaid bytecode op");
			break;
		}
		...
		if (!joint.displays.empty())
		{
			...
			for (const auto& d : joint.displays)
			{
				if (d.material != last.material)
				{
					...
					{
						if (mdl.getMaterials()[i].id == d.material)
							mid = i;
					}
					assert(mid != -1);
