...
void BoneEditor::drawBone(libcube::IBoneDelegate& d, bool leaf)
{
#if 0
...
#define BONE_PROP(ID, D) \
	...
	ImGui::PopItemWidth(); ImGui::PopID(); ImGui::NextColumn();
		...
		if ((d.canWrite(features::StandardBillboards) || d.canWrite(features::ExtendedBillboards)) && sel != static_cast<int>(d.getBillboard()))
			d.setBillboard(static_cast<libcube::IBoneDelegate::Billboard>(sel));

		if (samp.getBones().getNum() > 0 && samp.getBones().at(0).supportsBoneFeature(lib3d::BoneFeatures::SegmentScaleCompensation) != lib3d::Coverage::Unsupported)
		{
			bool b = d.getSSC();
...
#undef BONE_PROP
	...
	{	
		...
		{
			assert(samp.getBones().at(d.getChild(i)).getParent() == d.getId());
			drawBone(samp.getBones().at(d.getChild(i)));
		}
		ImGui::TreePop();
	}
#endif
...
void BoneEditor::draw(WindowContext* ctx) noexcept
{
	...
	if (!ImGui::Begin("BoneEditor", &bOpen) || !ctx)
		return;
#if 0
	bool use_ssc = samp.getBones().getNum() > 0 && samp.getBones().at(0).supportsBoneFeature(lib3d::BoneFeatures::SegmentScaleCompensation) != lib3d::Coverage::Unsupported;
	...
	if (ImGui::TreeNodeEx("(root)", ImGuiTreeNodeFlags_DefaultOpen))
	{
		...
		if (samp.getBones().getNum() > 0)
			drawBone(samp.getBones().at(0));
