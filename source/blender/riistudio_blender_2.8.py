...
def export_tex(texture, out_folder):
	tex_name = get_filename_without_extension(texture.image.name)
...
class BRRESTexturePanel(bpy.types.Panel):
	...
	def poll(cls, context):
		if context.active_node and context.active_node.bl_idname == 'ShaderNodeTexImage':
			return context.active_node

	def draw(self, context):
		tex = context.active_node
		layout = self.layout
		c_box = layout.box()
...
class JRESMaterialPanel(bpy.types.Panel):
	...
	def draw(self, context):
		...
		# Lighting
		box = layout.box()
		box.label(text="Lighting", icon='OUTLINER_OB_LIGHT')  # Might want to change icon here
...
def export_jres(
	...
	for Collection in bpy.data.collections:
		for Object in Collection.objects:
			...
			if Object.type == 'MESH':
				triangulated = Object.to_mesh(preserve_all_data_layers=False)
				# Triangulate:
				bm = bmesh.new()
				...
				for mat_index, mat in enumerate(triangulated.materials):
					...
					#print("LF: %u, M: %s, MA: %s" % (len(facepoints), mat, mat.node_tree.nodes.get('Image Texture') if mat else None))
					if len(facepoints) and mat and mat.node_tree.nodes.get('Image Texture'):
...
# src\exporters\brres\ExportBRRES.py
	...
	def draw(self, context):
		layout = self.layout
		box = layout.box()
		box.label(text="BRRES", icon='FILE_TICK')
		# Mesh
		box = layout.box()
		box.label(text="Mesh", icon='MESH_DATA')
		box.prop(self, "magnification", icon='VIEWZOOM')
		...
		# Root Transform
		box = layout.box()
		box.label(text="Root Transform", icon='FULLSCREEN_ENTER')
		row = box.row(align=True)
...
def register_tex():
	bpy.types.Node.brres_mode = EnumProperty(
		default='guided',
		items=(
	...
	bpy.types.Node.brres_guided_optimize = EnumProperty(
		name="Optimize for",
		items=(
			('quality', 'Quality', 'Optimize for quality'), ('filesize', 'Filesize', 'Optimize for lowest filesize')),
		default='filesize'
	)
	bpy.types.Node.brres_guided_color = EnumProperty(
		name="Color Type",
		items=(
	...
	bpy.types.Node.brres_guided_color_transparency = EnumProperty(
	...
	bpy.types.Node.brres_guided_grayscale_alpha = EnumProperty(
		name="Uses Alpha",
		default='use_alpha',
		items=(
	...
	bpy.types.Node.brres_manual_format = EnumProperty(
		items=texture_format_items
	)
	bpy.types.Node.brres_mipmap_mode = EnumProperty(
		items=(
	...
	bpy.types.Node.brres_mipmap_manual = IntProperty(
		name="#Mipmap",
		default=0
	)
	bpy.types.Node.brres_mipmap_minsize = IntProperty(
...
def register():
	bpy.types.TOPBAR_MT_file_export.append(brres_menu_func_export)
	
	register_tex()
	register_mat()

	# Texture Cache
	bpy.types.Node.jres_is_cached = BoolProperty(
...
def unregister():
	...
	bpy.types.TOPBAR_MT_file_export.remove(brres_menu_func_export)
