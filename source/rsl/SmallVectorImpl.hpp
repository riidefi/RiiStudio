...
void SmallVectorTemplateBase<T, TriviallyCopyable>::grow(size_t MinSize) {
  ...
  if (MinSize > this->SizeTypeMax()) {
    assert(!"SmallVector capacity overflow during allocation");
    exit(1);
  }
  ...
  if (this->capacity() == this->SizeTypeMax()) {
    assert(!"SmallVector capacity unable to grow");
    exit(1);
  }
...
template <typename T>
inline void swap(rsl::SmallVectorImpl<T>& LHS, rsl::SmallVectorImpl<T>& RHS) {
  LHS.swap(RHS);
}

/// Implement std::swap in terms of SmallVector swap.
template <typename T, unsigned N>
inline void swap(rsl::SmallVector<T, N>& LHS, rsl::SmallVector<T, N>& RHS) {
  LHS.swap(RHS);
}
