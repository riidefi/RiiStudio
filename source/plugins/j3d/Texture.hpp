...
struct TextureData
{
	...
	bool operator==(const TextureData& rhs) const {
		return mName == rhs.mName && mFormat == rhs.mFormat && bTransparent == rhs.bTransparent &&
			mWidth == rhs.mWidth && mHeight == rhs.mHeight && mPaletteFormat == rhs.mPaletteFormat && nPalette == rhs.nPalette &&
			ofsPalette == rhs.ofsPalette && mMinLod == rhs.mMinLod && mMaxLod == rhs.mMaxLod && mMipmapLevel == rhs.mMipmapLevel &&
			mData == rhs.mData;
	}
};
