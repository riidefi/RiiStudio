...
void SceneGraph::onRead(oishii::BinaryReader& reader, BMDOutputContext& ctx)
{
	...
	for (ByteCodeCmd cmd(reader); cmd.op != ByteCodeOp::Terminate; cmd.transfer(reader))
	{
		...
		case ByteCodeOp::Joint: {
			...
			{
				ctx.mdl.getJoint(ctx.jointIdLut[joint_stack.back()]).get().children.emplace_back(
		...
		case ByteCodeOp::Shape:
			if (mat < ctx.mdl.getMaterials().size())
				ctx.mdl.getJoint(ctx.jointIdLut[joint]).get().displays.emplace_back(
					ctx.mdl.getMaterial(mat).get().id, cmd.idx);
			break;
...
{
	SceneGraphNode(const ModelAccessor mdl)
	...
	void writeBone(oishii::v2::Writer& writer, const Joint& joint, const ModelAccessor mdl, u32& depth) const
	{
...
std::unique_ptr<oishii::v2::Node> SceneGraph::getLinkerNode(const ModelAccessor mdl)
{
