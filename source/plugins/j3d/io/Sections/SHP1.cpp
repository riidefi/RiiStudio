...
{
	SHP1Node(const ModelAccessor model)
		: mModel(model)
	{
		mId = "SHP1";
	...
	struct SubNode : public oishii::v2::Node
	{
		...
		SubNode(const ModelAccessor mdl, SubNodeID id, const SHP1Node& parent,int polyId=-1, int MPrimId = -1)
		...
		Result write(oishii::v2::Writer& writer) const noexcept
		{
			...
			case SubNodeID::MTXGrpHdr:
				break; // Children write
			case SubNodeID::_MTXGrpChild:
				for (int i = 0; i < mMdl.getShape(mPolyId).get().mMatrixPrimitives.size(); ++i)
				{
