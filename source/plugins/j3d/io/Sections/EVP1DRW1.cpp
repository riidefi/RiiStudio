...
void readEVP1DRW1(BMDOutputContext& ctx)
{
	...
	if (enterSection(ctx, 'DRW1'))
	{
		...
		ctx.mdl.get().mDrawMatrices.resize(reader.read<u16>() - envelopes.size()); // Bug in N's tooling corrected on runtime
		reader.read<u16>();
...
struct EVP1Node
{
	...
	struct SimpleEvpNode : public oishii::v2::Node
	{
		...
		SimpleEvpNode(const std::vector<DrawMatrix>& from, const std::vector<int>& toWrite, const std::vector<float>& weightPool, const ModelAccessor mdl= ModelAccessor{ nullptr })
	...
	struct MatrixSizeTable : public SimpleEvpNode
	{
		MatrixSizeTable(const EVP1Node& node)
			: SimpleEvpNode(node.mdl.get().mDrawMatrices, node.envelopesToWrite, node.weightPool)
		{
			mId = "MatrixSizeTable";
		}
	...
	struct MatrixInvBindTable : public SimpleEvpNode
	{
		MatrixInvBindTable(const EVP1Node& node, const ModelAccessor md)
			: SimpleEvpNode(node.mdl.get().mDrawMatrices, node.envelopesToWrite, node.weightPool, md)
		{
...
struct DRW1Node
{
	...
	struct MatrixTypeTable : public oishii::v2::Node {
		MatrixTypeTable(const Model& mdl)
			: mMdl(mdl)
		{
			mId = "MatrixTypeTable";
		}
