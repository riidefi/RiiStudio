...
void readVTX1(BMDOutputContext& ctx)
{
		...
		type = reader.read<gx::VertexBufferAttribute>())
	{
		...
		{
			auto& clr = ctx.mdl.get().mBufs.color[static_cast<size_t>(type) - static_cast<size_t>(gx::VertexBufferAttribute::Color0)];
			buf = &clr;
		...
		case gx::VertexBufferAttribute::TexCoord7: {
			auto& uv = ctx.mdl.get().mBufs.uv[static_cast<size_t>(type) - static_cast<size_t>(gx::VertexBufferAttribute::TexCoord0)];
			buf = &uv;
		...
		const auto ofs = g.start + ofsData[idx];
		{
			...
			case VBufferKind::color: {
				auto clr = reinterpret_cast<decltype(ctx.mdl.get().mBufs.color)::value_type*>(buf);
			...
			case VBufferKind::textureCoordinate: {
				auto uv = reinterpret_cast<decltype(ctx.mdl.get().mBufs.uv)::value_type*>(buf);
