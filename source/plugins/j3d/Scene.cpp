...
void Model::MatCache::propogate(Material& mat) {
	...
	for (int i = 0; i < mat.samplers.size(); ++i) {
		...
		// tmp.btiId = reinterpret_cast<MaterialData::J3DSamplerData*>(mat.samplers[i].get())->btiId;
		// update_section(samplers, tmp);
		update_section(samplers, *reinterpret_cast<MaterialData::J3DSamplerData*>(mat.samplers[i].get()));
	}
