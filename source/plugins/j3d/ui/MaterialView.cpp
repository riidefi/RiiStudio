...
void drawProperty(kpi::PropertyDelegate<Material>& delegate, J3DDataSurface) {
    ...
    if (ImGui::CollapsingHeader("Fog", ImGuiTreeNodeFlags_DefaultOpen)) {
        ...
        int orthoOrPersp = static_cast<int>(fog.type) >= static_cast<int>(Fog::Type::OrthographicLinear) ? 0 : 1;
        ...
        ImGui::Combo("Function", &fogType, "None\0Linear\0Exponential\0Quadratic\0Inverse Exponential\0Quadratic\0");
    ...
    if (ImGui::CollapsingHeader("Light Colors (Usually ignored by games)", ImGuiTreeNodeFlags_DefaultOpen)) {
        
        int i = 0;
        for (auto& clr : delegate.getActive().lightColors) {
            clr_f32 = clr;
            ImGui::ColorEdit4((std::string("Light Color ") + std::to_string(i)).c_str(), clr_f32);
...
void install() {
    kpi::PropertyViewManager::getInstance().addPropertyView<Material, J3DDataSurface>();
}
