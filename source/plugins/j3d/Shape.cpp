...
glm::vec4 Shape::getClr(u64 idx) const
{
	...
	auto raw = static_cast<libcube::gx::ColorF32>((libcube::gx::Color)mMdl.mBufs.color[0].mData[idx]);
...
void Shape::addTriangle(std::array<SimpleVertex, 3> tri)
{
	...
	if (mMatrixPrimitives.back().mPrimitives.empty() ||
		mMatrixPrimitives.back().mPrimitives.back().mType != libcube::gx::PrimitiveType::Triangles)
		mMatrixPrimitives.back().mPrimitives.emplace_back(libcube::gx::PrimitiveType::Triangles, 0);
