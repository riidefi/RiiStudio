...
struct MatrixPrimitive
{
	...
	bool operator==(const MatrixPrimitive& rhs) const {
		return mCurrentMatrix == rhs.mCurrentMatrix && mDrawMatrixIndices == rhs.mDrawMatrixIndices && mPrimitives == rhs.mPrimitives;
	}
};
struct ShapeData
{
	...
	bool operator==(const ShapeData& rhs) const {
		return id == rhs.id && mode == rhs.mode && bsphere == rhs.bsphere && bbox == rhs.bbox &&
			mMatrixPrimitives == rhs.mMatrixPrimitives && mVertexDescriptor == rhs.mVertexDescriptor;
	}
};
