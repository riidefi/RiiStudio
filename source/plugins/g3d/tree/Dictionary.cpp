...
void Dictionary::calcNode(u32 entry_idx)
{
	...
	while (entry.mId <= current.mId && current.mId < prev.mId)
	{
		...
		{
			entry.mId = calc_brres_id(current.mName.c_str(), current.mName.size(), entry.mName.c_str(), entry.mName.size());
...
void DictionaryNode::read(oishii::BinaryReader& reader, u32 groupStartPosition)
{
	mId = reader.read<u16>();
