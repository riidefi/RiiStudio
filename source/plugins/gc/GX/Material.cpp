...
void calcTexMtx_Basic(glm::mat4& dst, float scaleS, float scaleT, float rotation, float translationS, float translationT,
	float centerS, float centerT, float centerQ)
{
	...
	dst[0][0] = scaleS * cosR;
	dst[1][0] = scaleS * -sinR;
	dst[2][0] = translationS + centerS + scaleS * (sinR * centerT - cosR * centerS);

	dst[0][1] = scaleT * sinR;
	dst[1][1] = scaleT * cosR;
	dst[2][1] = translationT + centerT + -scaleT * (-sinR * centerS + cosR * centerT);
}

void calcTexMtx_Maya(glm::mat4& dst, float scaleS, float scaleT, float rotation, float translationS, float translationT)
{
	const auto theta = rotation * 3.141592f;
	const auto sinR = sin(theta);
	const auto cosR = cos(theta);
...
void computeNormalMatrix(glm::mat4& dst, const glm::mat4& m, bool isUniformScale)
{
	if (dst != m)
		dst = m;
...
void texEnvMtx(glm::mat4& dst, float scaleS, float scaleT, float transS, float transT)
{
	dst[0][0] = scaleS;
	dst[1][0] = 0.0;
	dst[2][0] = 0.0;
...
glm::mat3x4 GCMaterialData::TexMatrix::compute(const glm::mat4& mdl, const glm::mat4& mvp)
{
