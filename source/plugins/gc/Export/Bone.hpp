...
struct IBoneDelegate : public riistudio::lib3d::Bone
{
    ...
    void copy(riistudio::lib3d::Bone& to) override
    {
        riistudio::lib3d::Bone::copy(to);
        IBoneDelegate* bone = reinterpret_cast<IBoneDelegate*>(&to);
        if (bone)
        {
            if (bone->supportsBoneFeature(riistudio::lib3d::BoneFeatures::StandardBillboards) == riistudio::lib3d::Coverage::ReadWrite)
                bone->setBillboard(getBillboard());
            if (bone->supportsBoneFeature(riistudio::lib3d::BoneFeatures::ExtendedBillboards) == riistudio::lib3d::Coverage::ReadWrite)
