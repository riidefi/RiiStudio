...
void writeImageStb(const char* filename, STBImage type, int x, int y, int channel_component_count, const void* data)
{
	...
	{
	case STBImage::PNG:
		stbi_write_png(filename, x, y, channel_component_count, data, x * channel_component_count);
		break;
	case STBImage::BMP:
	...
	case STBImage::JPG:
		...
		break;
	case STBImage::HDR:
		stbi_write_hdr(filename, x, y, channel_component_count, static_cast<const float*>(data));
		break;
	default:
