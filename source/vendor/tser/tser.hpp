//
// RiiStudio note: Modified to not interpret uint8 / int8 data as a printable character!
//
#include <stdint.h>
...
namespace tser{
    ...
    constexpr inline decltype(auto) print(std::ostream& os, T&& val) {
        ...
        else if constexpr (std::is_constructible_v<std::string, T> || std::is_same_v<V, char>)
            os << "\"" << val << "\"";
