...
inline std::shared_ptr<const DocumentMemento> setNext(const IDocumentNode& node, std::shared_ptr<const DocumentMemento> record) {
	DocumentMemento transient = *record.get();
	// Compare the actual data of the node
	if (record->JustData.get() == nullptr || !node.compareJustThisNotChildren(*record->JustData.get())) {
	...
	std::set_difference(record->lut.begin(), record->lut.end(), node.lut.begin(), node.lut.end(),
		std::back_inserter(deleted_folders));
	std::set_difference(node.lut.begin(), node.lut.end(), record->lut.begin(), record->lut.end(),
	...
	return transient == *record.get() ? record : std::make_shared<const DocumentMemento>(transient);
}



inline void rollback(IDocumentNode& node, std::shared_ptr<const DocumentMemento> record) {
	// Compare the actual data of the node
