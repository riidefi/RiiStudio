...
struct IPropertyView {
    ...
    virtual void draw(kpi::IDocumentNode& active, std::vector<kpi::IDocumentNode*> affected,
...
public:
    ...
    virtual const T& getActive() const {
        return mActive;
    }
    ...
    #define KPI_PROPERTY(delegate, before, after, val) \
        delegate.property(before, after, \
...
public:
    PropertyDelegate(T& active, std::vector<T*> affected, kpi::History& history, const kpi::IDocumentNode& transientRoot)
        : mActive(active), mAffected(affected), mHistory(history), mTransientRoot(transientRoot)
    {}
};
...
struct PropertyViewImpl final : public IPropertyView {
    ...
    void draw(kpi::IDocumentNode& active, std::vector<kpi::IDocumentNode*> affected,
        kpi::History& history, kpi::IDocumentNode& root) override
    {
...
struct PropertyViewManager {
    ...
    template<typename T>
    void forEachView(T func, kpi::IDocumentNode& active) {
        for (auto& it : mViews) {
            if (!it->isInDomain(&active)) continue;
