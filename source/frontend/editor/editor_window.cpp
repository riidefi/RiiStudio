...
struct PropertyEditor : public StudioWindow
{
	...
	void draw_() override
	{
		...
		for (std::size_t i = 0; i < mMats->size(); ++i) {
			if (mMats->isSelected(i)) mats.push_back(&mMats->at<kpi::IDocumentNode>(i));
		}
		if (mats.empty()) {
			ImGui::Text("No material is selected");
			return;
		}
		auto* _active = dynamic_cast<libcube::IGCMaterial*>((*mMats)[mMats->getActiveSelection()].get());
		ImGui::Text("%s %s (%u)", _active->getName().c_str(), mats.size() > 1 ? "..." : "", static_cast<u32>(mats.size()));

		if (ImGui::BeginTabBar("Pane")) {
			int i = 0;
			std::string title;
			manager.forEachView([&](kpi::IPropertyView& view) {
