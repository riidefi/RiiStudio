		...
		if (ImGui::BeginMenu("Experimental")) {
			if (ImGui::MenuItem("Convert to BMD") && ed != nullptr) {
				...
				for (int fr_i = 0; fr_i < from_models->size(); ++fr_i) {
					...
					// info
					// Bufs (automate)
					bmd_model.get().mBufs.norm.mQuant.comp.normal = libcube::gx::VertexComponentCount::Normal::xyz;
					// drawmtx (skip)
					...
					// materials
					{
						...
						for (int m_i = 0; m_i < from_mats->size(); ++m_i) {
							...
							for (int i = 0; i < before.size(); ++i) {
								auto simp = std::make_unique<j3d::MaterialData::J3DSamplerData>();
					...
					// shapes
					{
						...
						for (int m_i = 0; m_i < from_shapes->size(); ++m_i) {
							...
							bmd_shape.mVertexDescriptor.calcVertexDescriptorFromAttributeList();
							...
							for (int i = 0; i < from_shape.getNumMatrixPrimitives(); ++i) {
								...
								// No multi mtx yet
								for (int j = 0; j < from_shape.getMatrixPrimitiveNumIndexedPrimitive(i); ++j) {
									const auto& prim = from_shape.getMatrixPrimitiveIndexedPrimitive(i, j);
									auto& p = bmd_mp.mPrimitives.emplace_back(prim);
									// Remap vtx indices
									for (auto& v : p.mVertices) {
										for (u32 x = 0; x < (u32)libcube::gx::VertexAttribute::Max; ++x)
										{
											if (!(vcd.mBitfield & (1 << x))) continue;
											auto& bufs = bmd_model.get().mBufs;
											switch (static_cast<libcube::gx::VertexAttribute>(x))
											{
											case libcube::gx::VertexAttribute::PositionNormalMatrixIndex:
											...
											case libcube::gx::VertexAttribute::Position:{
												auto pos = from_shape.getPos(v[libcube::gx::VertexAttribute::Position]);
												auto found = std::find(bufs.pos.mData.begin(), bufs.pos.mData.end(), pos);
												if (found == bufs.pos.mData.end()) {
													bmd_model.get().mBufs.pos.mData.push_back(pos);
													v[libcube::gx::VertexAttribute::Position] = bmd_model.get().mBufs.pos.mData.size() - 1;
												} else {
													v[libcube::gx::VertexAttribute::Position] = found - bufs.pos.mData.begin();
												}
												break;
												}
											case libcube::gx::VertexAttribute::Color0:
											{
												const auto scolor = from_shape.getClr(v[libcube::gx::VertexAttribute::Color0]);
												...
												v[libcube::gx::VertexAttribute::Color0] = bmd_model.get().mBufs.color[0].mData.size() - 1;
												break;
											}
											...
											{
												const auto chan = x - static_cast<int>(libcube::gx::VertexAttribute::TexCoord0);
												const auto attr = static_cast<libcube::gx::VertexAttribute>(x);
												const auto data = from_shape.getUv(chan, v[attr]);

												bmd_model.get().mBufs.uv[chan].mData.push_back(data);
												v[static_cast<libcube::gx::VertexAttribute>(x)] = bmd_model.get().mBufs.uv[chan].mData.size() - 1;


												break;
											}
											case libcube::gx::VertexAttribute::Normal:
												bmd_model.get().mBufs.norm.mData.push_back(from_shape.getNrm(v[libcube::gx::VertexAttribute::Normal]));
												v[libcube::gx::VertexAttribute::Normal] = bmd_model.get().mBufs.norm.mData.size() - 1;
												break;
											default:
				...
				attachEditorWindow(std::make_unique<EditorWindow>(std::move(bmd_state), "__conv.bmd"));
			}
			ImGui::EndMenu();
		}
#endif
