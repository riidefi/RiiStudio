...
struct DataMesh
{
    ...
    virtual px::InternalClassMirror* get(const std::string& id) = 0;
    virtual void declare(const std::string& id, px::RichName name) = 0;
    virtual void enqueueHierarchy(px::MirrorEntry entry) = 0;
...
public:
	...
	ReflectionMesh(std::unique_ptr<DataMesh> mesh)
		: mDataMesh(std::move(mesh))
	{}
	
	inline DataMesh& getDataMesh()
	{
		return *mDataMesh.get();
	}
	...
	public:
		...
		template<typename T>
		T* castToImmediateParent(T* in, int index)
		{
	...
	private:
		...
		ReflectionInfoHandle(InternalClassMirror* mirror = nullptr, DataMesh* mesh = nullptr)
	...
	virtual void findParentOfType(std::vector<void*>& out, void* in, const std::string& info, const std::string& key) = 0;

	template<typename TDst, typename TDynamic>
	inline std::vector<TDst*> findParentOfType(TDynamic&& type)
	{
		std::vector<void*> out;
		findParentOfType(out, type.mBase, std::string(type.mType), std::string(TDst::TypeInfo.namespacedId));
